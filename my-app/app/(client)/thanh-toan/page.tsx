...
export default function ThanhToan() {
  ...
  useEffect(() => {
    if (listSP.length === 0) {
      alert(
        "Bạn chưa chọn sản phẩm nào. Vui lòng chọn sản phẩm rồi mới thanh toán"
      );
      router.push("/");
    }
  }, [listSP, router]);
  ...
  const submitDuLieu = async () => {
    ...
    const selectedValue = Object.values(paymentRef.current).find(
      (el) => el.checked
    )?.value;
    ...
    try {
      ...
      if (res.ok) {
        ...
        Swal.fire(
          "Lỗi",
          data.thong_bao || "Đã xảy ra lỗi khi tạo đơn hàng",
          "error"
        );
      }
    } catch (err) {
  ...
  return (
    ...
    <div className="checkout-container">
      <div className="checkout-left">
        ...
        {user && (
          <form id="checkout-form">
            ...
            <div
              ref={thongbaoRef}
              className="text-red-500 font-bold italic text-2xl"
            ></div>
          </form>
        )}
      ...
      <div className="checkout-right">
        ...
        <div className="items">
          {listSP.map((item, idx) => (
            <div className="order-item" key={idx}>
              ...
              <span>
                {(item.gia_mua * item.so_luong).toLocaleString("vi")}VNĐ
              </span>
            </div>
          ))}
        </div>
        ...
        {voucherMessage && (
          <p style={{ color: voucherSuccess ? "green" : "red" }}>
            {voucherMessage}
          </p>
        )}
        ...
        <h3 style={{ padding: "10px 0" }}>
          Tổng cộng:{" "}
          <span>
            {(subtotal + shippingFee - discountAmount).toLocaleString("vi")}VNĐ
          </span>
        </h3>
